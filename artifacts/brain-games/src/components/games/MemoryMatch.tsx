...
const CONFIGS: Record<Difficulty, DiffConfig> = {
  ...
  medium: { pairs: 10, cols: 5, timer: 120,  label: "Medium", scoreBase: 1800, gridClass: "grid-cols-5", cardClass: "h-13 sm:h-16",   color: "text-yellow-400 border-yellow-600/40 bg-yellow-900/20" },
...
export default function MemoryMatch({ onComplete, seed }: MemoryMatchProps) {
            ...
            return (
              ...
              >
                ...
                <div className="text-right">
                  <div className="text-xs text-zinc-500 font-medium uppercase tracking-wider">Score up to</div>
  ...
  const iconSize = difficulty === "hard" ? "w-5 h-5 sm:w-6 sm:h-6" : difficulty === "medium" ? "w-6 h-6 sm:w-7 sm:h-7" : "w-7 h-7 sm:w-9 sm:h-9";

  return (
    <div className="w-full max-w-2xl mx-auto flex flex-col items-center gap-4">
      {/* HUD */}
      <div className="flex justify-between w-full font-mono text-xs sm:text-sm tracking-widest text-zinc-400">
        ...
        <AnimatePresence>
          {comboFlash && (
            <motion.div
              ...
              className="text-yellow-400 font-black text-sm sm:text-base tracking-widest"
            >
              ⚡ COMBO ×{combo}!
            </motion.div>
          )}
      ...
      {/* Grid */}
      <div className={`grid ${cfg.gridClass} gap-2 sm:gap-3 w-full`}>
        {cards.map((card, index) => {
          ...
          return (
            <motion.div
              ...
              className={`relative ${cfg.cardClass} cursor-pointer`}
            ...
            >
              ...
              >
                {/* Back */}
                <div className="absolute inset-0 backface-hidden bg-zinc-800/90 border border-zinc-700/60 rounded-xl hover:bg-zinc-700/90 hover:border-primary/30 transition-colors shadow-md" />
                {/* Front */}
                <div
